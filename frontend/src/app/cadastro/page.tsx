...
export default function CadastroPage() {
  ...
  const handleCadastro = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const resultado = await cadastrarLoja(formData);
    ...
    <div className="min-h-screen bg-[#0b1120] flex items-center justify-center relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md p-8 bg-[#1e293b]/60 border border-slate-700/50 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-xl relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 mt-2 text-sm">Crie sua loja para começar a gerenciar produtos e preços</p>
        </div>
        ...
        <form onSubmit={handleCadastro} className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-medium text-slate-300 ml-1">Nome da loja</label>
            <input
              name="nome_loja"
              type="text"
              required
          ...
          <button
            type="submit"
            disabled={carregando}
            className="w-full mt-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 disabled:opacity-50 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_25px_rgba(6,182,212,0.5)]"
          >
