...
import { fetchCategoriasDoToken } from './actions';

// Obs: mantemos esse componente para facilitar evoluções.
// Por enquanto, não usamos supabase; carregamos via API do Django.
...
export default function ModalNovoProdutoWithCats() {
  ...
  const carregarCategorias = async () => {
    ...
    try {
      // Buscar categorias via server action (lê o cookie httpOnly e chama a API com Bearer)
  ...
  const defaultCategoriaId = useMemo(() => (categorias[0]?.id ? String(categorias[0].id) : ''), [categorias]);
